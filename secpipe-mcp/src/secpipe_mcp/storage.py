"""SecPipe MCP Server - Local project storage.

Lightweight project storage for managing `.secpipe/` directories,
execution results, and project configuration. Extracted from the
former secpipe-runner storage module.

Storage is placed directly in the project directory as `.secpipe/`
...
logger = logging.getLogger("secpipe-mcp")

#: Name of the SecPipe storage directory within projects.
SECPIPE_DIR_NAME: str = ".secpipe"
...
class LocalStorage:
    """Local filesystem storage backend for SecPipe.
    ...
    def _get_project_path(self, project_path: Path) -> Path:
        """Get the .secpipe storage path for a project.

        :param project_path: Path to the project directory.
        :returns: Storage path (.secpipe inside project).

        """
        return project_path / SECPIPE_DIR_NAME

    def init_project(self, project_path: Path) -> Path:
        """Initialize storage for a new project.

        Creates a .secpipe/ directory inside the project for storing
    ...
    def scan_artifacts(
        ...
        Compares the current state of .secpipe/output/ against the existing
    ...
    def save_report(
        ...
        """Save a generated report to .secpipe/reports/.
    ...
    #: Directory containing built-in skill packs shipped with SecPipe.
    ...
    def list_skills(self, project_path: Path) -> list[dict[str, Any]]:
        ...
        for skill_dir in self._skill_dirs(project_path):
            for yaml_path in sorted(skill_dir.glob("*.yaml")):
                ...
                is_project = ".secpipe" in str(yaml_path.parent)
