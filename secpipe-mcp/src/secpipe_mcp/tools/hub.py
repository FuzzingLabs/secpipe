"""MCP Hub tools for SecPipe MCP server.

This module provides tools for interacting with external MCP servers
through the SecPipe hub. AI agents can:
...
from secpipe_common.hub import HubExecutor, HubServerConfig, HubServerType
...
def _get_hub_executor() -> HubExecutor:
    ...
    if not settings.hub.enabled:
        msg = "MCP Hub is disabled. Enable it via SECPIPE_HUB__ENABLED=true"
        raise ToolError(msg)
...
async def execute_hub_tool(
    ...
    to subsequent tool calls. The host path is .secpipe/output/.
